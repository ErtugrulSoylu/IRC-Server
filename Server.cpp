...
void Server::handle_message(int i) {
    ...
    if (users[i].connected == true && users[i].username != "")
    {
        ...
        for (int j = 0; j < valread + 1; j++)
        {
            if(buffer[j] == '\n')
            {
                buffer[valread-1] = '\0';
                shelves[i] += buffer;
                for (int j = 0; j <= (int)shelves[i].length() + valread; j++)
                {
    ...
    else if (users[i].connected == true && users[i].username == "")
    {
        ...
        send(sd , "\nYou are redirecting to Genel channel\n", strlen("\nYou are redirecting to Genel channel\n"), 0 );
        std::string join_msg = "\033[1;31m" + users[i].nickname + "\033[0m entered.\n";
...
bool Server::check_if_command(int user_id) {
    ...
    std::cout << users[user_id].username << ": " << cmd << " command used." << std::endl;
    return true;
}
...
void Server::join(std::vector<std::string> params, int user_id) {
    ...
    {
        send(client_socket[user_id], "\nyou already in this channel\n", strlen("\nyou already in this channel\n"), 0);
        return ;
    }

    if (ch_num >= 0) {
        std::string join_msg = "\033[1;31m" + users[user_id].nickname +"\033[0m has joined to channel\n";
...
void Server::list(std::vector<std::string> params, int user_id) {
    if (params.size() != 0)
    {
        send(client_socket[user_id], "\nThere are more parameter than expected\n", strlen("\nThere are more parameter than expected\n"), 0);
        return ;
    }
...
void Server::msg(std::vector<std::string> params, int user_id) {
    ...
    std::string str = "";
    str += "\n\033[1;31m" + users[user_id].nickname + "\033[0m send a message: ";
    for (int i = 0; i < MAX_CLIENTS; i++) {
...
void Server::nick(std::vector<std::string> params, int user_id) {
    if (params.size() != 1)
    {
        send(client_socket[user_id], "\nnot enought parameter count\n", strlen("\nnot enought parameter count\n"), 0);
        return ;
    }
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (users[i].nickname == params[0]){
            send(client_socket[user_id], "\nThis nick already taken.\n", strlen("\nThis nick already taken.\n"), 0);
            return ;
        }
    }
